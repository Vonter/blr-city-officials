<script lang="ts">
  ...
  function queryAllDistrictsForCoordinates(coord: { lat: number; lon: number }) {
  ...
  async function handleSubmit() {
    ...
    const pairs = coordinates
      ...
      .filter((coord): coord is { lat: number; lon: number } => 
    ...
    return results.map(result => {
      ...
      result.containingBoundaries.forEach(boundary => {
        ...
        if (boundaryDef) {
          ...
          const official = officials.find(o => o.Area === boundaryName && o.Department === id);
          if (official) {
      ...
      ...results.map(row => {
        const values = headers.map(header => {
          ...
          if (official?.name) {
            ...
            if (official.designation) cellContent += ` (${official.designation}) - `;
        ...
        <div class="p-6">
          ...
          {#if results.length > 0}
            <div class="overflow-x-auto">
              <table class="min-w-full divide-y divide-gray-200">
                <thead>
                  <tr>
                    <th class="px-4 py-3 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider sticky top-0">
                      Coordinates
                    </th>
                    ...
                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                      ...
                      {#each Object.values(layers) as boundary}
                        <td class="px-4 py-3 text-sm text-gray-700">
                          <div class="space-y-2">
                            <div class="font-medium">{row[boundary.name_long] || '—'}</div>
                            {#if typeof row[`${boundary.name_long}_official`] === 'object' && row[`${boundary.name_long}_official`]?.name}
                              <div class="pl-2 border-l-2 border-gray-200 space-y-1">
                                <div class="font-medium text-gray-900">{(row[`${boundary.name_long}_official`] as OfficialDetails).name}</div>
                                <div class="text-gray-500">{(row[`${boundary.name_long}_official`] as OfficialDetails).designation}</div>
                                <div>
                                  <a href="mailto:{(row[`${boundary.name_long}_official`] as OfficialDetails).email}" 
                                     class="text-blue-600 hover:underline">
                                    {(row[`${boundary.name_long}_official`] as OfficialDetails).email}
                                  </a>
                                </div>
                                <div class="text-gray-600">{(row[`${boundary.name_long}_official`] as OfficialDetails).phone}</div>
                              </div>
                            {/if}
                          </div>
                        </td>
          ...
          {:else}
            <div class="text-center py-12 text-gray-500">
              No results yet. Enter coordinates and submit to see the boundary information.
            </div>
          {/if}
        </div>
      </div>
